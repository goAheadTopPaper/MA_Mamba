...
class Model(nn.Module):
    ...
    def __init__(self, configs):
        ...
        self.encoder1 = Encoder(
            [
                EncoderLayer(
                        Mamba(
                            d_model=configs.d_model,  # Model dimension d_model
                            d_state=configs.d_state*4,  # SSM state expansion factor
                            d_conv=2,  # Local convolution width
                            expand=1,  # Block expansion factor)
                        ),
                        Mamba(
        ...
        self.projector = nn.Linear(configs.d_model, configs.pred_len, bias=False)
    ...
    def forecast(self, x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        enc_out1, attns1 = self.encoder(enc_out, attn_mask=None)
        enc_out2, attns2 = self.encoder1(enc_out, attn_mask=None)
        outputs.append(enc_out1)
        outputs.append(enc_out2)
        enc_out = sum(outputs) / len(outputs)
        #enc_out = enc_out1
        # affine_out = self.affine(enc_out)
        # B N E -> B N S -> B S N 
